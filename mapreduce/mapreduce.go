...
func InitMapReduce(nmap int, nreduce int,
	...
	mr.WorkersPool = make(chan string)
	mr.Workers = make(map[string]*WorkerInfo)

	fmt.Printf("Start listening for new workers...\n")
	go mr.ListenForNewWorker()

	return mr
}

func (mr *MapReduce) ListenForNewWorker() {
	for {
		var worker string
		select {
		case worker = <-mr.registerChannel:
			...
			fmt.Printf("Worker %s registered and added to the pool\n", worker)
		}
	}
}
